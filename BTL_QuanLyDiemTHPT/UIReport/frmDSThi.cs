    ...
    public partial class frmDSThi : Form
    {
        ...
        private void btnIn_Click(object sender, EventArgs e)
        {
            using(SqlConnection conn = Connection.DBConnection.getConnection())
            {
                ...
                cmd.Connection = conn;
                cmd.CommandText = "DSThi";
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@TenKhoi", txtKhoi.Text.Trim());
                cmd.Parameters.AddWithValue("@TenMon", txtTenMon.Text.Trim());
                adapter.SelectCommand = cmd;
                adapter.Fill(table);
                rptDSThi report = new rptDSThi();
                report.SetDataSource(table);
                rptDanhSachThi.ReportSource = report;
                rptDanhSachThi.Refresh();
            }
        }
    }
}
